        ...
        <div className="flex -space-x-2">
          {row.metadata?.beforePhoto && (
            <div className="h-8 w-8 rounded-md border-2 border-background bg-muted overflow-hidden">
              <img src={row.metadata.beforePhoto} alt="Sebelum" className="h-full w-full object-cover" />
            </div>
          )}
          {row.metadata?.afterPhoto && (
            <div className="h-8 w-8 rounded-md border-2 border-background bg-muted overflow-hidden">
              <img src={row.metadata.afterPhoto} alt="Sesudah" className="h-full w-full object-cover" />
            </div>
          )}
  ...
  return (
    <div className="space-y-6 p-4 md:p-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Kelola jadwal kebersihan dan pantau kebersihan lingkungan asrama.
          </p>
        </div>
        {canEdit && (
          <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
            ...
            <DialogContent className="sm:max-w-[500px]">
              <DialogHeader>
                <DialogTitle>Jadwal Kebersihan Baru</DialogTitle>
              </DialogHeader>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="border-l-4 border-l-primary">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Total Aktivitas</p>
        ...
        <Card className="border-l-4 border-l-emerald-500">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Selesai</p>
        ...
        <Card className="border-l-4 border-l-amber-500">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Proses/Menunggu</p>
      ...
      <Card className="shadow-sm">
        <CardHeader>
          <CardTitle className="text-lg">Buku Catatan Aktivitas</CardTitle>
          <CardDescription>Daftar lengkap tugas kebersihan dan status pelaksanaannya.</CardDescription>
        </CardHeader>
        <CardContent>
          <DataTable 
            columns={columns} 
            data={activities} 
            searchPlaceholder="Cari berdasarkan aktivitas atau area..."
          />
        </CardContent>
      </Card>
                    ...
                    <div className="space-y-2">
                      <div className="aspect-square rounded-lg bg-muted flex items-center justify-center overflow-hidden border-2 border-dashed border-muted-foreground/20">
                        {selectedActivity.metadata?.beforePhoto ? (
                          <img src={selectedActivity.metadata.beforePhoto} alt="Sebelum" className="h-full w-full object-cover" />
                        ) : (
                          <span className="text-xs text-muted-foreground">Belum ada foto sebelum</span>
                        )}
                      </div>
                      <p className="text-[10px] text-center uppercase font-bold text-muted-foreground">Sebelum</p>
                    </div>
                    <div className="space-y-2">
                      <div className="aspect-square rounded-lg bg-muted flex items-center justify-center overflow-hidden border-2 border-dashed border-muted-foreground/20">
                        {selectedActivity.metadata?.afterPhoto ? (
                          <img src={selectedActivity.metadata.afterPhoto} alt="Sesudah" className="h-full w-full object-cover" />
                        ) : (
                          <span className="text-xs text-muted-foreground">Belum ada foto sesudah</span>
                        )}
                      </div>
                      <p className="text-[10px] text-center uppercase font-bold text-muted-foreground">Sesudah</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    {selectedActivity.comments?.length ? (
                      ...
                      <div className="text-center py-6 bg-muted/30 rounded-lg">
                        <p className="text-sm text-muted-foreground italic">Belum ada umpan balik.</p>
                      </div>
                    )}
                    ...
                    {(isMudir || isTopManagement) && (
                      <div className="pt-2">
                        <textarea 
                          placeholder="Tambahkan umpan balik langsung untuk aktivitas ini..."
